...
Before ANY code edits, complete the detailed design check internally:
...
7) Ownership check (single source of truth; duplicated or synchronized values)
8) Cleanup boundary (relevant cleanup included; related work explicitly deferred)

Present only a Short Design Snapshot to the user:

- Goal: one concise bullet
- Architecture: responsibility, ownership, and contract impact in one concise bullet
- Safety: primary risk and scope boundary in one concise bullet
- Validation: test and regression coverage in one concise bullet

Keep the detailed design check available for execution, but do not make the user read it unless they request the expanded version.
...
### Incremental Architecture Hygiene
- Leave the touched area at least as clear as it was before the change.
- Clean up coupling when it directly affects the current behavior, testability, or tuning.
- Keep cleanup scoped to the active responsibility boundary; do not expand into unrelated systems.
- When multiple components control the same behavior or tuning value, identify one authoritative owner and make other components consume that contract.
- Prefer behavior-oriented parameters (for example, melee engagement distance or turn speed) over implementation-oriented values that must be manually synchronized.
- Separate decision-making, state, execution, and presentation when a touched component currently owns several of them.
- Do not add another responsibility to a controller already near or above 300 lines without proposing an extraction.
- If safe cleanup would materially expand the active issue, document the coupling, add relevant regression coverage, and create or recommend a focused follow-up issue.
- Do not perform speculative rewrites. Preserve working behavior through small, independently testable steps.
...
This trigger explicitly authorizes Codex to complete the local-to-draft-PR workflow without asking for separate confirmation at each step.

Execute in order:
1. Inspect the current branch, working tree, and diff; confirm the files match the approved scope.
2. Run or verify the relevant EditMode and PlayMode tests. Stop and report failures unless the user already supplied trustworthy passing results for the unchanged diff.
3. If currently on `main`, create an appropriately named feature branch before committing.
4. Stage only files belonging to the approved work. Never include unrelated user changes.
5. Commit with the Conventional Commits format below.
6. Push the feature branch to the remote.
7. Create a draft pull request with the title supplied separately from the body. The PR body must start with `## Why` and use the template below.
8. Apply the appropriate labels when permissions and tooling allow.
9. Confirm that CI checks were created or started by the push or PR. If the repository requires a specific safe manual workflow trigger, start it and report what was triggered.
10. Report the branch, commit, PR URL, initial CI state, suggested squash merge message, and post-merge cleanup checklist.

Safety boundaries:
- Do not force-push, merge, deploy, bypass required checks, or delete branches under this trigger.
- Do not broaden scope or fix unrelated CI failures without a new approval.
- If authentication, push, PR creation, or CI startup fails, report the exact blocker and preserve the completed local work.
- Task-specific trigger workflows supplement the normal workflow. Authorization granted by a trigger applies only to the actions explicitly listed under that trigger.
...
- In the created PR body, every checklist item in the PR template MUST be checked as `[x]`.
...
- <any labeled assumptions the agent made during implementation>
...
- `READY TO CLOSE: [summary]` — validate, publish a draft PR, and start CI
